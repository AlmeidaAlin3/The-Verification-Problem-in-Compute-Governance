...
#define SAMPLE_TIME_MS_DEFAULT  10
...
typedef struct {
    ...
    int sample_time_ms;
    int max_samples;
    int cpu_core;          // -1 = do not pin affinity
    int nice_value;        // e.g. -20
} sampler_config_t;

extern sampler_config_t config;

// Buffers
extern uint64_t *timestamps_us;      // raw monotonic timestamps, in microseconds
extern uint32_t *power_mw_data;      // power in mW
extern uint32_t *sm_clock_data;      // MHz
extern uint32_t *mem_clock_data;     // MHz
extern uint32_t *gpu_util_data;      // %
extern uint32_t *pcie_tx_data;       // KB/s
extern uint32_t *temperature_data;   // C
